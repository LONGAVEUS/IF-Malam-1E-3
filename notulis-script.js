...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function setupEventListeners() {
    ...
    document.addEventListener("click", function (event) {
      ...
      if (searchContainer && !searchContainer.contains(event.target) && event.target !== pesertaSearch) {
        hideSearchResults();
      }
    });
  ...
  function updateSelectedPesertaList() {
    ...
    selectedPesertaList.innerHTML = selectedPeserta
      .map(
        (peserta, index) => `
  ...
  function updateEditSelectedPesertaList() {
    ...
    editSelectedPesertaList.innerHTML = window.selectedPesertaEdit
      .map(
        (peserta) => `
            <div class="peserta-item" data-id="${peserta.id}">
                <div class="peserta-info">
  ...
  window.openEditModal = async function (notulenId) {
    try {
      ...
      if (data.peserta_details && Array.isArray(data.peserta_details)) {
        window.selectedPesertaEdit = data.peserta_details
          .map((p) => ({
  ...
  function handleEditFormSubmit(event) {
    ...
    fetch("notulen_rapat.php", {
      ...
      .then((data) => {
        ...
        if (data.includes("Notulen berhasil diperbarui!") || data.includes("berhasil") || data.includes("success")) {
    ...
    try {
      ...
      if (data.length === 0) {
        ...
        data.forEach((item, index) => {
          ...
          const waktuKonfirmasi = item.waktu_konfirmasi ? new Date(item.waktu_konfirmasi).toLocaleString("id-ID") : "-";
